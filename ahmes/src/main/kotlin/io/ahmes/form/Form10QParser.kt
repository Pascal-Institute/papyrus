...
/**
 * 10-Q Quarterly Report Parser
 *
 * 10-Q is a quarterly financial report containing:
 * - Part I: Financial Information (Financial Statements, MD&A)
 * - Part II: Other Information (Legal Proceedings, Risk Factors)
 */
class Form10QParser : BaseSecReportParser<Form10QParseResult>(SecReportType.FORM_10Q) {
    ...
    override fun extractSections(content: String): Map<String, String> {
        val itemPatterns = listOf(
            Regex("(?i)part\\s+i[^i].*?item\\s+1[.:\\-\\s]+(financial\\s+statements|condensed)", RegexOption.IGNORE_CASE),
            Regex("(?i)part\\s+i[^i].*?item\\s+2[.:\\-\\s]+(management'?s\\s+discussion|md&a)", RegexOption.IGNORE_CASE),
            Regex("(?i)part\\s+i[^i].*?item\\s+3[.:\\-\\s]+(quantitative|market\\s+risk)", RegexOption.IGNORE_CASE),
            Regex("(?i)part\\s+i[^i].*?item\\s+4[.:\\-\\s]+(controls|procedures)", RegexOption.IGNORE_CASE),
            Regex("(?i)part\\s+ii.*?item\\s+1[.:\\-\\s]+(legal\\s+proceedings)", RegexOption.IGNORE_CASE),
            Regex("(?i)part\\s+ii.*?item\\s+1a[.:\\-\\s]+(risk\\s+factors)", RegexOption.IGNORE_CASE),
            Regex("(?i)part\\s+ii.*?item\\s+2[.:\\-\\s]+(unregistered\\s+sales)", RegexOption.IGNORE_CASE),
            Regex("(?i)part\\s+ii.*?item\\s+6[.:\\-\\s]+(exhibits)", RegexOption.IGNORE_CASE)
        )
    ...
    private fun extractExecutiveSummary(mdaContent: String): String {
        val summaryPattern = Regex("(?i)(overview|executive\\s+summary)[.:\\-\\s]*", RegexOption.IGNORE_CASE)
        val match = summaryPattern.find(mdaContent) ?: return mdaContent.take(1000)

        val nextSectionPattern = Regex("(?i)(results\\s+of\\s+operations|three\\s+months)", RegexOption.IGNORE_CASE)
    ...
    private fun extractResultsOfOperations(mdaContent: String): String {
        ...
        val endPattern = Regex("(?i)(liquidity|capital\\s+resources|financial\\s+condition)", RegexOption.IGNORE_CASE)
    ...
    private fun extractQuarterInfo(content: String): Pair<String?, String?> {
        ...
        if (match != null) {
            ...
            val quarter = when (month.uppercase()) {
                "MARCH", "MAR" -> "Q1"
                "JUNE", "JUN" -> "Q2"
        ...
        val altPattern = Regex("(?i)(q[1-4]|first|second|third)\\s+quarter\\s+(?:of\\s+)?(\\d{4})", RegexOption.IGNORE_CASE)
        ...
        if (riskSection.contains("no material changes", ignoreCase = true)) {
            riskFactors.add(
                RiskFactor(
                    ...
                    summary = "No material changes to risk factors since last annual report",
