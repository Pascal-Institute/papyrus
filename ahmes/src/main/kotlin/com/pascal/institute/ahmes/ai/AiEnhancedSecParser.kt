...
object AiEnhancedSecParser {
    ...
    fun enhance(
        ...
        val entities = if (options.enableEntityExtraction) {
        ...
        // Sentiment analysis
        val sentiment = if (options.enableSentimentAnalysis && sections.isNotEmpty()) {
        ...
        val sectionClassifications = if (options.enableSectionClassification && sections.isNotEmpty()) {
        ...
        // Risk analysis
        val riskAnalysis = if (options.enableRiskAnalysis) {
            val riskSection = sections.entries.find {
                it.key.lowercase().contains("risk")
            }?.value ?: ""
        ...
        // Document summary
        val documentSummary = if (options.enableDocumentSummary) {
        ...
        val aiConfidence = if (confidenceScores.isNotEmpty()) {
            confidenceScores.fold(BigDecimal.ZERO) { acc, bd -> acc.add(bd) }
    ...
    private fun extractTextSections(text: String): Map<String, String> {
        ...
        sectionPatterns.forEachIndexed { index, (pattern, name) ->
            ...
            if (match != null) {
                ...
                val endIndex = if (nextPattern != null) {
                    Regex(nextPattern, RegexOption.IGNORE_CASE).find(upperText, startIndex + 1)?.range?.first
                        ?: text.length
                } else {
                    text.length
                }
    ...
    private fun extractRiskFactorTexts(riskSection: String): List<String> {
        ...
        if (risks.isEmpty()) {
            riskSection.split("\n\n")
                ...
                .take(10)
                .forEach { risks.add(it.take(1000)) }
        }
