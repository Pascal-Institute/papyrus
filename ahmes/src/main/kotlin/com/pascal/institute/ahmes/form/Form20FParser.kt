...
/**
 * 20-F Foreign Company Annual Report Parser
 *
 * 20-F is an annual report for foreign companies, similar to 10-K but:
 * - IFRS or US GAAP financial statements
 * - Foreign company specific information
 */
class Form20FParser : BaseSecReportParser<Form20FParseResult>(SecReportType.FORM_20F) {

    override fun parseHtml(htmlContent: String, metadata: SecReportMetadata): Form20FParseResult {
        val cleanedContent = cleanHtml(htmlContent)
        return parseContent(cleanedContent, htmlContent, metadata)
    }

    override fun parseText(textContent: String, metadata: SecReportMetadata): Form20FParseResult {
    ...
    override fun extractSections(content: String): Map<String, String> {
        val sectionPatterns = listOf(
            "Item 3" to Regex("(?i)item\\s+3[.:\\-\\s]+(key\\s+information)", RegexOption.IGNORE_CASE),
            "Item 4" to Regex("(?i)item\\s+4[.:\\-\\s]+(information\\s+on\\s+the\\s+company)", RegexOption.IGNORE_CASE),
            "Item 5" to Regex("(?i)item\\s+5[.:\\-\\s]+(operating\\s+and\\s+financial)", RegexOption.IGNORE_CASE),
            "Item 8" to Regex("(?i)item\\s+8[.:\\-\\s]+(financial\\s+information)", RegexOption.IGNORE_CASE),
            "Item 11" to Regex("(?i)item\\s+11[.:\\-\\s]+(quantitative.*?market\\s+risk)", RegexOption.IGNORE_CASE)
        )
    ...
    private fun extractAccountingStandard(content: String): String? {
        return when {
            ...
            content.contains("International Financial Reporting Standards", ignoreCase = true) -> "IFRS"
            content.contains("US GAAP", ignoreCase = true) -> "US GAAP"
            content.contains("Generally Accepted Accounting Principles", ignoreCase = true) -> "US GAAP"
            else -> null
        }
    }
