 ...
 *
 * Uses DJL deep learning models to extract structured financial entities
 * from unstructured SEC filing text.
 */
object SecEntityExtractor {
    ...
    private val fiscalPeriodPattern = Regex(
        """(FY|Q[1-4]|fiscal\s+year|quarter)\s*(\d{4}|\d{2})?""",
        RegexOption.IGNORE_CASE
    )

    private val tickerPattern = Regex(
        """\b([A-Z]{1,5})\b(?:\s*\([^)]*\))?"""
    )

    // Common SEC metric names
    private val metricNames = setOf(
    ...
    fun answerQuestion(question: String, context: String): QAAnswer {
        return try {
            val model = DjlModelManager.getQuestionAnsweringModel()
            if (model != null) {
    ...
    private fun extractMonetaryValues(text: String): List<FinancialEntity> {
        return monetaryPattern.findAll(text).map { match ->
    ...
    private fun extractPercentages(text: String): List<FinancialEntity> {
        return percentagePattern.findAll(text).map { match ->
            FinancialEntity(
                text = match.value.trim(),
    ...
    private fun extractDates(text: String): List<FinancialEntity> {
        return datePattern.findAll(text).map { match ->
            FinancialEntity(
                text = match.value.trim(),
    ...
    private fun extractFiscalPeriods(text: String): List<FinancialEntity> {
        return fiscalPeriodPattern.findAll(text).map { match ->
            FinancialEntity(
                text = match.value.trim(),
    ...
    private fun extractExecutiveName(text: String, title: String): String? {
        val pattern = Regex("""([A-Z][a-z]+\s+[A-Z][a-z]+(?:\s+[A-Z][a-z]+)?),?\s*(?:our\s+)?$title""", RegexOption.IGNORE_CASE)
