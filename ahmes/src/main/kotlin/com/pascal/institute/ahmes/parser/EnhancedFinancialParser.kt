...
object EnhancedFinancialParser {
    ...
    fun parseFromSecDocument(secDoc: SecDocumentText): List<ExtendedFinancialMetric> {
        logger.info { "Starting SEC document parsing: company=${secDoc.companyName}, fiscalYear=${secDoc.fiscalYear}, sections=${secDoc.sections.size}" }

        val metrics = mutableListOf<ExtendedFinancialMetric>()

        for (section in secDoc.sections) {
            logger.debug { "Parsing section: type=${section.type}, contentLength=${section.content.length}" }
    ...
    fun parseFinancialMetrics(content: String): List<ExtendedFinancialMetric> {
        ...
        try {
            ...
            logger.debug { "Table parsing extracted ${tableMetrics.size} metrics from ${tables.size} tables" }
        ...
        val foundCategories = metrics.map { it.category }.toSet()
        logger.debug { "Searching for ${allPatterns.size} patterns (skipping ${foundCategories.size} already found)" }
        ...
        logger.info { "Financial metrics parsing complete: ${result.size} unique metrics (${metrics.size} before deduplication)" }
        return result
    }
    ...
    fun parseRiskFactors(content: String): List<RiskFactor> {
        ...
        val riskSection =
                extractSection(cleanText, listOf("RISK FACTORS", "Item 1A"))
        ...
        for (pattern in riskPatterns) {
            ...
            for (match in matches.take(15)) {
                ...
                logger.trace { "Risk factor: category=$category, severity=$severity, title='${title.take(50)}...'" }
            }
        }

        val result = risks.distinctBy { it.title }.take(10)
        logger.info { "Risk factor parsing complete: ${result.size} unique risks (${risks.size} before deduplication)" }
        return result
    }
    ...
    fun calculateRatios(metrics: List<ExtendedFinancialMetric>): List<FinancialRatio> {
        ...
        logger.debug { "Ratio categories: ${ratios.groupBy { it.category }.mapValues { it.value.size }}" }
        return ratios
    }
        ...
        private fun assessRiskSeverity(text: String): RiskSeverity {
                ...
                return when {
                        lowerText.contains("material adverse") ||
                                lowerText.contains("significant risk") -> RiskSeverity.HIGH
