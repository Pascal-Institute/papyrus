...
import com.pascal.institute.ahmes.model.ExtendedFinancialMetric
import com.pascal.institute.ahmes.model.MetricCategory
import com.pascal.institute.ahmes.model.MetricUnit
...
object InlineXbrlExtractor {
    ...
    private val factExpr = xpath.compile(
    ...
    private fun parseContexts(root: XdmNode): Map<String, XbrlContext> {
        ...
        for (ctx in contextExpr.load().apply { contextItem = root }.evaluate().nodes()) {
    ...
    private fun extractFacts(root: XdmNode): List<XbrlFact> {
        ...
        for (node in factExpr.load().apply { contextItem = root }.evaluate().nodes()) {
            ...
            facts.add(
                XbrlFact(
                    ...
                    decimals = decimals,
                    scale = scale,
                    source = describeSource(concept, contextRef, unitRef),
                )
            )
        }
    ...
    private fun extractConceptAndText(node: XdmNode): Pair<String, String>? {
        val concept = node.attrAny("name")?.takeIf { it.isNotBlank() }
    ...
    private fun extractConceptAndTextFallback(el: org.jsoup.nodes.Element): Pair<String, String>? {
        val concept = el.attrAnyCase("name")?.takeIf { it.isNotBlank() }
    ...
    private fun mapConceptToMetric(conceptRaw: String): Pair<String, MetricCategory>? {
        ...
        val rule = conceptRules.firstOrNull { r -> r.matchers.any { it(concept) } }
            ?: return null
    ...
    private fun describeSource(concept: String, contextRef: String?, unitRef: String?): String {
        return buildString {
            append("iXBRL:")
            append(concept)
    ...
    private fun stringChild(node: XdmNode, localName: String): String? {
        val selector = xpath.compile("./*[local-name()='$localName']/text()").load()
        selector.contextItem = node
        return selector.evaluate().firstText()
    }
