 ...
 *
 * Uses DJL deep learning models to analyze sentiment and risk in SEC filings. Provides
 * investment-relevant insights from financial text.
 */
object SecSentimentAnalyzer {
    ...
    private val severityKeywords =
            mapOf(
                    RiskSeverity.CRITICAL to
                            listOf(
                                    "material adverse",
                                    "significant harm",
                                    "bankruptcy",
                    ...
                    RiskSeverity.LOW to
                            listOf("minor", "limited", "manageable", "unlikely", "remote")
            )
        ...
        return try {
            DjlModelManager.withSentimentPredictor { predictor ->
                texts.map { text ->
                    ...
                    val result = predictor.predict(truncated)
                    classificationToSentiment(truncated, result)
                }
            }
        } catch (e: Exception) {
            logger.warn(
                    "Batch sentiment analysis failed, falling back to individual calls: ${e.message}"
            )
            texts.map { analyzeSentiment(it) }
        }
    }
    ...
    fun getDocumentSentimentSummary(sections: Map<String, String>): DocumentSentimentSummary {
        val sectionSentiments = sections.mapValues { (_, content) -> analyzeSentiment(content) }
        ...
        return DocumentSentimentSummary(
                ...
                positiveRatio =
                        BigDecimal(overallPositive)
                                .divide(BigDecimal(total), 4, RoundingMode.HALF_UP)
                                .toPlainString(),
                negativeRatio =
                        BigDecimal(overallNegative)
                                .divide(BigDecimal(total), 4, RoundingMode.HALF_UP)
                                .toPlainString(),
                neutralRatio =
                        BigDecimal(overallNeutral)
                                .divide(BigDecimal(total), 4, RoundingMode.HALF_UP)
                                .toPlainString(),
    ...
    private fun classificationToSentiment(
            text: String,
            classifications: Classifications
    ): SentimentResult {
        val best = classifications.best<Classifications.Classification>()
        val sentiment =
                when (best.className.uppercase()) {
        ...
        val details =
                classifications.items<Classifications.Classification>().associate {
                    it.className to
                            BigDecimal(it.probability)
    ...
    private fun ruleBasedSentiment(text: String): SentimentResult {
        ...
        return SentimentResult(
                ...
                sentiment = sentiment,
                confidence =
                        BigDecimal(confidence).setScale(4, RoundingMode.HALF_UP).toPlainString(),
    ...
    private fun assessSeverity(riskText: String, sentiment: SentimentResult): RiskSeverity {
        ...
        return when {
            sentiment.isNegative && sentiment.getConfidenceBigDecimal() > BigDecimal("0.8") ->
                    RiskSeverity.HIGH
