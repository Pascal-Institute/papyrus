...
object FinancialPrecision {
    ...
    /**
     * Create a monetary amount from a double value WARNING: Only use this when parsing external
     * data. Prefer string-based creation.
     */
    ...
    /**
     * Parse SEC-style financial values with unit scaling e.g., "1,234.56" in millions =
     * 1,234,560,000
     */
    fun parseSecValue(
            valueStr: String,
    ...
    fun calculatePercentageChange(
        ...
        val percentChange =
                change.divide(previous.abs(), CALCULATION_SCALE, RoundingMode.HALF_UP)
                        .multiply(BigDecimal("100"))
                        .setScale(2, RoundingMode.HALF_UP)
    ...
    /**
     * Validate financial amount for plausibility AGENTS.MD Principle 4: Financial Validation Checks
     */
    ...
    /** Check if two monetary amounts are equal within tolerance */
    fun areEqual(
            amount1: MonetaryAmount,
            amount2: MonetaryAmount,
            tolerance: BigDecimal = BigDecimal("0.01")
    ): Boolean {
        val diff = amount1.subtract(amount2).abs().number.numberValue(BigDecimal::class.java)
...
/**
 * Custom exception for financial state anomalies AGENTS.MD Principle 4: Prevent hallucinations and
 * impossible results
 */
