...
object SecTableParser {
    ...
    private val extendedSectionPatterns =
            mapOf(
                    StatementType.INCOME_STATEMENT to
                            listOf(
                    ...
                    StatementType.BALANCE_SHEET to
                            listOf(
                    ...
                    StatementType.CASH_FLOW_STATEMENT to
                            listOf(
                    ...
                    StatementType.COMPREHENSIVE_INCOME to
                            listOf(
                    ...
                    StatementType.EQUITY_STATEMENT to
                            listOf(
        ...
        for (pattern in patterns) {
            ...
            if (startIdx != -1) {
                ...
                for (endPattern in endPatterns) {
                    val possibleEnd =
                            lowerContent.indexOf(endPattern, startIdx + pattern.length + 100)
    ...
    private fun parseTextTableRows(section: String, type: StatementType): List<TableRow> {
        ...
        for (line in lines) {
            ...
            if (label.all {
                        it.isDigit() || it == ',' || it == '.' || it.isWhitespace() || it == '$'
                    }
            )
                    continue
            ...
            rows.add(
                    TableRow(label = label, values = values, isTotal = isTotal, category = category)
            )
        }
    ...
    private fun mapLabelToCategory(label: String, statementType: StatementType): MetricCategory? {
        ...
        return when {
            // 수익 관련
            lowerLabel == "total revenue" || lowerLabel == "total revenues" ->
            ...
            lowerLabel.contains("cost of goods sold") || lowerLabel == "cogs" ->
            ...
            lowerLabel == "accounts receivable" || lowerLabel.contains("trade receivables") ->
            ...
            lowerLabel.contains("stockholders' equity") ||
                    lowerLabel.contains("shareholders' equity") -> MetricCategory.TOTAL_EQUITY
            lowerLabel.contains("total stockholders' equity") -> MetricCategory.TOTAL_EQUITY
            lowerLabel == "retained earnings" || lowerLabel == "accumulated deficit" ->
                    MetricCategory.RETAINED_EARNINGS

            // 현금흐름
            lowerLabel.contains("net cash provided by operating") ->
            ...
            // 주당 지표
            lowerLabel.contains("basic earnings per share") || lowerLabel == "basic eps" ->
                    MetricCategory.EPS_BASIC
            lowerLabel.contains("diluted earnings per share") || lowerLabel == "diluted eps" ->
                    MetricCategory.EPS_DILUTED
            lowerLabel == "earnings per share" || lowerLabel == "eps" -> MetricCategory.EPS_BASIC

            // 비용
            lowerLabel.contains("research and development") || lowerLabel == "r&d expense" ->
                    MetricCategory.RD_EXPENSE
            lowerLabel.contains("selling, general") || lowerLabel == "sg&a" ->
    ...
    private fun detectPeriodType(period: String?): PeriodType? {
        ...
        return when {
            lower.contains("q1") ||
                    lower.contains("q2") ||
                    lower.contains("q3") ||
                    lower.contains("q4") -> PeriodType.QUARTERLY
