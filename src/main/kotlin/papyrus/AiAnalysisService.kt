...
/**
 * OpenRouter AI 분석 서비스
 * 무료 LLM API를 사용하여 재무 데이터 분석
 */
object AiAnalysisService {
     ...
     * 재무 데이터 산업 비교 분석
     */
    suspend fun compareWithIndustry(
        companyName: String,
        ratios: List<FinancialRatio>
    ): String = withContext(Dispatchers.IO) {
        if (!isConfigured()) {
        ...
        return@withContext try {
            val ratioSummary = ratios.joinToString("\n") { 
     ...
     * 투자 의사결정 지원 분석
     */
    suspend fun generateInvestmentAdvice(
        companyName: String,
        ...
        return@withContext try {
            ...
            val ratiosText = analysis.ratios.take(8).joinToString("\n") { "- ${it.name}: ${it.formattedValue}" }
    ...
    private fun buildAnalysisPrompt(
        ...
        val ratiosText = ratios.take(10).joinToString("\n") { 
        ...
        val requestBody = """
            {
                ...
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": ${json.encodeToString(kotlinx.serialization.serializer<String>(), prompt)}
                    }
                ],
                "temperature": 0.3,
                "max_tokens": 2000,
    ...
    private fun parseAiResponse(response: String): AiAnalysisResult {
        return try {
            ...
            json.decodeFromString<AiAnalysisResult>(jsonString).copy(
                success = true,
                provider = "OpenRouter",
                model = FREE_MODEL
            )
            ...
            AiAnalysisResult(
                ...
                keyInsights = listOf(
                    "AI 분석이 완료되었으나 구조화하지 못했습니다.",
                    "응답 내용: ${response.take(200)}"
                ),
