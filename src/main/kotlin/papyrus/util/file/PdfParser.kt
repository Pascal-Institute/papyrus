...
object PdfParser {
     ...
     *
     * Recognizes column alignment and adds appropriate separators to better preserve tabular data
     * from PDFs.
     */
            ...
            PDDocument.load(file).use { document ->
                val stripper =
                        PDFTextStripper().apply {
                            sortByPosition =
                                    true // Position-based sorting to preserve table structure
                ...
                val sectionMarkers =
                        listOf(
                                ...
                                SectionMarker(
                                        ...
                                        SecSectionType.COMPREHENSIVE_INCOME
                                ),
                                SectionMarker(
                                        "CONSOLIDATED STATEMENTS OF SHAREHOLDERS",
                                        SecSectionType.EQUITY_STATEMENT
                                ),
                                SectionMarker(
                ...
                for (marker in sectionMarkers) {
                    ...
                    if (startIdx != -1) {
                        ...
                        val endIdx =
                                findNextSectionEnd(
                                        fullText,
                                        startIdx + marker.pattern.length,
                                        sectionMarkers
                                )
                        val sectionText =
                                fullText.substring(startIdx, minOf(endIdx, startIdx + 30000))
                ...
                val companyName =
                        extractCompanyName(fullText, document.documentInformation.title ?: "")
    ...
    private fun detectDocumentType(text: String): SecDocumentType {
        ...
        return when {
            upperText.contains("FORM 10-K") || upperText.contains("ANNUAL REPORT") ->
                    SecDocumentType.FORM_10K
            upperText.contains("FORM 10-Q") || upperText.contains("QUARTERLY REPORT") ->
                    SecDocumentType.FORM_10Q
            upperText.contains("FORM 8-K") -> SecDocumentType.FORM_8K
            upperText.contains("FORM 20-F") -> SecDocumentType.FORM_20F
            upperText.contains("PROXY STATEMENT") || upperText.contains("DEF 14A") ->
                    SecDocumentType.PROXY
    ...
    private fun extractFiscalYear(text: String): String? {
        ...
        val patterns =
                listOf(
                        Regex(
                                """fiscal\s+year\s+ended?\s+([A-Za-z]+\s+\d{1,2},?\s+\d{4})""",
                                RegexOption.IGNORE_CASE
                        ),
                        Regex(
                                """for\s+the\s+year\s+ended?\s+([A-Za-z]+\s+\d{1,2},?\s+\d{4})""",
                                RegexOption.IGNORE_CASE
                        ),
    ...
    private fun detectTableType(text: String): TableType {
        ...
        return when {
            upperText.contains("OPERATIONS") ||
                    upperText.contains("INCOME") && !upperText.contains("COMPREHENSIVE") ->
                    TableType.INCOME_STATEMENT
            upperText.contains("BALANCE") ||
                    upperText.contains("ASSETS") && upperText.contains("LIABILITIES") ->
     ...
     *
     * SEC PDF documents usually follow this format: Label Value1 Value2 Value3
     *
    ...
    private fun inferCategory(label: String): String {
        ...
        return when {
            // Revenue
            lowerLabel.contains("revenue") ||
                    lowerLabel.contains("net sales") ||
                    lowerLabel == "sales" -> "REVENUE"

            // Cost of revenue
            lowerLabel.contains("cost of") &&
                    (lowerLabel.contains("revenue") ||
                            lowerLabel.contains("sales") ||
                            lowerLabel.contains("goods")) -> "COST_OF_REVENUE"

            // Profit indicators
            lowerLabel == "gross profit" || lowerLabel == "gross margin" -> "GROSS_PROFIT"
            lowerLabel.contains("operating income") ||
                    lowerLabel.contains("income from operations") -> "OPERATING_INCOME"
            lowerLabel == "net income" ||
                    lowerLabel.contains("net income (loss)") ||
            ...
            lowerLabel.contains("long-term debt") || lowerLabel.contains("long term debt") ->
                    "LONG_TERM_DEBT"
            lowerLabel.contains("accounts payable") -> "ACCOUNTS_PAYABLE"

            // Equity
            lowerLabel.contains("equity") ||
                    lowerLabel.contains("shareholders") ||
                    lowerLabel.contains("stockholders") -> "EQUITY"
            lowerLabel.contains("retained earnings") ||
                    lowerLabel.contains("accumulated deficit") -> "RETAINED_EARNINGS"

            // Cash flow
            lowerLabel.contains("operating activities") ||
                    lowerLabel.contains("cash from operations") -> "OPERATING_CASH_FLOW"
...
data class SecSection(val type: SecSectionType, val content: String, val startIndex: Int)

data class SectionMarker(val pattern: String, val type: SecSectionType)
