...
object BookmarkManager {
     ...
     * 티커 북마크 추가
     */
    fun addBookmark(ticker: TickerEntry, notes: String = "", tags: List<String> = emptyList()): Boolean {
        ...
        val newBookmark = BookmarkedTicker(
            cik = ticker.cik,
    ...
    fun updateBookmarkNotes(cik: Int, notes: String): Boolean {
        val data = loadBookmarks()
        val updated = data.bookmarks.map { 
            if (it.cik == cik) it.copy(notes = notes) else it 
        }
    ...
    fun updateBookmarkTags(cik: Int, tags: List<String>): Boolean {
        val data = loadBookmarks()
        val updated = data.bookmarks.map { 
            if (it.cik == cik) it.copy(tags = tags) else it 
        }
    ...
    fun addToRecentlyViewed(cik: Int) {
        ...
        val updatedBookmarks = data.bookmarks.map {
            if (it.cik == cik) {
                it.copy(lastViewed = LocalDateTime.now().format(DateTimeFormatter.ISO_LOCAL_DATE_TIME))
            } else it
        }
        
        saveBookmarks(data.copy(
     ...
     * 컬렉션 생성
     */
    fun createCollection(name: String, description: String = "", color: String = "#1A73E8"): Boolean {
        val data = loadBookmarks()
    ...
    fun addToCollection(collectionName: String, cik: Int): Boolean {
        val data = loadBookmarks()
        val updated = data.collections.map { collection ->
