...
/**
 * NewsApi - Fetches company news from various sources
...
object NewsApi {
    ...
    suspend fun getCompanyNews(ticker: String, companyName: String, limit: Int = 10): CompanyNews? {
        return try {
            ...
            val articles = newsResponse.feed?.take(limit)?.map { item ->
                NewsArticle(
    ...
    private fun getMockNews(ticker: String, companyName: String): CompanyNews {
        val mockArticles = listOf(
            NewsArticle(
                ...
                description = "$companyName exceeded analyst expectations with strong revenue growth and improved profit margins.",
                imageUrl = null
            ),
            NewsArticle(
                ...
                description = "Several major investment firms have raised their price targets for $ticker following the company's strategic announcement.",
                imageUrl = null
            ),
            NewsArticle(
                ...
                description = "The partnership is expected to drive growth in key market segments and enhance the company's competitive position.",
                imageUrl = null
            ),
            NewsArticle(
                title = "Market Update: $ticker Shares Rise on Strong Sector Performance",
                url = "https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=$ticker",
                source = "Bloomberg",
                publishedAt = "Dec 28, 2025 11:20",
                description = "The broader sector rally has lifted $ticker shares, with investors showing increased confidence in the company's fundamentals.",
                imageUrl = null
            ),
            NewsArticle(
                title = "$companyName Continues Innovation Push with New Product Launch",
                url = "https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=$ticker",
                source = "TechCrunch",
                publishedAt = "Dec 25, 2025 13:00",
                description = "The new product line addresses emerging market needs and positions $companyName for future growth opportunities.",
                imageUrl = null
            )
        )
